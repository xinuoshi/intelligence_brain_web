    ...
    <div class="earchTraffic flexColumn flexNum">
      ...
      <div class="earchTrafficBody flexNum flexRow">
        <div class="flexNum leftEarchTraffic flexColumn">
            <!-- 企业违法排行榜 表格 -->
            <div class="tableOffic flexNum zpEarch" id="tableOffic"  @mouseenter="stopScroll" @mouseleave="continueScroll">
                <div class="tableOffictitle">企业违法排行榜</div>
                <el-table :data="tableData" id="table"  height="90%">
                    ...
                    <el-table-column prop="num" label="违法次数(次)"></el-table-column>
                </el-table>
                <el-table :data="firstData" id="table" style="background-color: rgb(8, 31, 63);">
                    ...
                    <el-table-column prop="address" label="行政区域"></el-table-column>
                    <el-table-column prop="num" label="违法数量(个)"></el-table-column>
                </el-table>
            </div>   
            <!-- 违法天气类型统计 饼图-->
        ...
        <div style="flex:3" class="zpEarch flexColumn">  
          ...
          <!-- 违法行为分布统计 折线图-->
          <div id="Columns2" class="flexNum" @mouseenter="stopInter" @mouseleave="keepInter"></div>
        </div>
        <div class="flexNum flexColumn">  
          ...
          <!-- <div id="Caketype1" class="zpEarch flexNum"> -->
                <!-- <div class="title">违法人员属地统计（人）</div>
...
import TrafficOrderAnalysis from "../../../JSON/TrafficOffenceAnalysis.json";
...
export default {
  ...
  methods: {
    ...
    //   //   alert("是否导出");
        ...
        // let gis = document.getElementsByClassName("earchTrafficBody")[0];
        // let ww = gis.clientWidth;
        // let hh = gis.clientHeight;
        // // console.log(ww,hh)
        // // html2canvas(gis, { scale:2,foreignObjectRendering:false,logging: false, useCORS: true }).then(function(canvas) {
        ...
        //     saveAs(blob, "交通事故分析" + time + ".png");
        // });
        this.$router.push({
                path: "/download",
                query: {
                    timer: this.timer,
                    url: "sjh2/strategysupport/safty/illAnalysis/"
                }
            });
    },
    // 24小时违法数量统计
        ...
        series: [
          {
            ...
            data: [this.accidentCarNum, this.accidentCarNum, this.accidentCarNum, this.accidentCarNum, this.accidentCarNum, this.accidentCarNum]
          },
          {
            itemStyle: {
              normal: {
        ...
        yAxis: [
          {
            ...
            min: 0,
            max: this.accidentCompareNumber[4][0],
            type: "value"
          },
          {
            type: "value",
            ...
            series: [{
                ...
                areaStyle:{
                     ...
                     color:{
                        ...
                        r: 0.5,
                        colorStops: [{
                            offset: 0, color: "rgb(150,148,21)" // 0% 处的颜色
                        }, {
                            offset: 1, color: "rgb(236,255,10)" // 100% 处的颜色
                        }],
                        globalCoord: false // 缺省为 false
                    }
                }
    ...
    getTrafficNumber(timer) {
          let tablevalue = TrafficOrderAnalysis.illRegionRank.data;
          if(tablevalue.length === 0) {
              this.$showSimpleMessage("交通违法数量统计暂无数据", "error");
              return;
          }
          tablevalue.map(val => {
            val.name = val.area;
    ...
    // 24小时统计
    getTrafficDay(timer) {
            let userData = TrafficOrderAnalysis.duration.data;
    ...
    // 违法类型分布统计
    accidentType(timer) {
          let eventType = TrafficOrderAnalysis.illType.data;
          let accidentTypeNumber = [];
          let legend = [];
          let accidentTypeN = [];
          let illegalList = this.$store.state.illegal;
          if(eventType.length === 0) {
              this.$showSimpleMessage("违法行为分布暂无数据", "error");
          } else {
                eventType.map(val => {
    ...
    accidentCar(timer) {
          ...
          let req = TrafficOrderAnalysis.vehicleLocation.data;
          let num = 0;
          if(req.length !== 0) {
            req.map(val => {
                if(val.count > num) {
              ...
              this.$showSimpleMessage("违法机动车属地统计暂无数据", "error");
          };          
        this.accidentCarNum = num + 10;
      ...
      let params = {
        ...
        area: ""
          ...
          for (let i = 1; i <= length; i++) {
            ...
            qoq.push(result.qoq || 0);
            yoy.push(result.yoy || 0);
          ...
          accidentCompareNumber.push(back);
          this.accidentCompareNumber = accidentCompareNumber;
          this.Caketype2();
    },
    // 违法人员户口属地统计
    async accidentPerson(timer) {
          let req = TrafficOrderAnalysis.driverLocation.data;
          let driverLocation = [];
          let count = [];
          let num = 0;
           if(req.length === 0) {
    ...
    accidentDriverSex(timer) {
          let req = TrafficOrderAnalysis.sex.data;
          let driverLocation = [];
          req.map(val => {
    ...
    illegalDriverAge(timer) {
          let req = TrafficOrderAnalysis.age.data;
          let driverLocation = [];
          let count = [];
          let ageGroup = [];
          if(req.length > 0) {
            req.map(val => {
              ...
              this.$showSimpleMessage("违法驾驶员年龄统计暂无数据", "error");
          };            
          driverLocation.push(count);
          driverLocation.push(ageGroup);
          this.driverAgeArr = driverLocation;
          this.driverAge("违法驾驶员年龄统计（人）","age",count,ageGroup);
    },
    // 违法驾驶员违章次数统计
    illegalTime(timer) {
          let req = TrafficOrderAnalysis.driverVioNum.data;
          let driverLocation = [];
          let count = [];
          let violation = [];
          if(req.length > 0) {
              ...
              this.$showSimpleMessage("违法驾驶员违章次数统计暂无数据", "error");
          };            
          driverLocation.push(count);
          driverLocation.push(violation);
          this.illegalTimeArr = driverLocation;
          this.driverTime("num","违法驾驶员违章次数统计（人）",count,violation,["#206fc0", "#20c09c", "#247e44", "#d18523", "#6b20c0"],"人");
    },
    //违法驾驶员驾龄统计
    illDriverDriveAge(timer) {
          let req = TrafficOrderAnalysis.driveage.data;
          let driverLocation = [];
          let count = [];
          let driveAgeGroup = [];
          let driveAgeArr = [{
          ...
          if(req.length > 0) {
            ...
            // });
              ...
              this.$showSimpleMessage("违法驾驶员驾龄统计暂无数据", "error");
          };            
        //   driverLocation.push(count);
        //   driverLocation.push(driveAgeGroup);
          this.driverDirveAgeArr = driverLocation;
          this.driverDirveAge('违法驾驶员驾龄统计（人）',"driveAge", [
                    {
                        value : count,
                        name : '违法驾驶员驾龄统计（人）',
                        lineStyle: {
    ...
    //    违法驾驶员累计扣分统计
    driverHistory(timer) {
          let req = TrafficOrderAnalysis.score.data;
          let driverLocation = [];
          let dataS = [];
          let dataY = [];
          let dataArr = [{
          ...
          if(req.length > 0) {
            ...
            // });
              ...
              this.$showSimpleMessage(" 违法驾驶员累计扣分统计暂无数据", "error");
          };          
          this.driverDirveAge(' 违法驾驶员累计扣分统计（人）',"score", [
                    {
                        value : dataS,
                        name : ' 违法驾驶员累计扣分统计（人）',
                        lineStyle: {
    ...
    // 违法驾驶员历史违法统计
    driverCount(timer) {
          let req = TrafficOrderAnalysis.driverAcdNum.data;
          let driverLocation = [];
          let dataS = [];
          let dataY = [];
          if(req.length > 0) {
              ...
              this.$showSimpleMessage("违法驾驶员历史违法次数统计暂无数据", "error");
          };           
          this.driverTime("historyNum","违法驾驶员历史违法统计（起）",dataS,dataY,["#206fc0", "#20c09c", "#247e44", "#d18523", "#6b20c0"],"起");
    },
    // 违法机动车类型统计
    driverCarType(timer) {
      let carTypeList = this.$store.state.carType;
          let req = TrafficOrderAnalysis.carType.data;
          let driverLocation = [];
          let dataY = [];
          let dataS = [];
          if(req.length > 0) {
            req.map(val => {
              ...
              this.$showSimpleMessage("违法机动车类型统计暂无数据", "error");
          };           
          let dataO,dataT;
          let start = 0;
          ...
          let type = setInterval(()=>{
              if(this.carNatureScroll) {
                ...
                this.driverAge("违法机动车类型统计（辆）","carType",dataO,dataT,"辆");
              };
          },2000);
          this.type = type;
    },
    // 驾驶机动车颜色统计
    carColor(timer) {
          let req = TrafficOrderAnalysis.vehicleColour.data;
          let driverLocation = [];
          let dataY = [];
          let dataS = [];
          ...
          if(req.length > 0) {
            req.map(val => {
              ...
              this.$showSimpleMessage("违法机动车颜色统计暂无数据", "error");
          };  
           let dataO,dataT;
          let start = 0;
          let length = dataS.length;
          ...
          let type = setInterval(()=>{
              if(this.carNatureScroll) {
                ...
                dataO = this.$tools.deepCopy(dataS).splice(start, 5);
                dataT = this.$tools.deepCopy(dataY).splice(start, 5);
                this.driverAge("违法机动车颜色统计（辆）","carColor",dataO,dataT,"辆");
              }
          },2000);
          this.car = type;
            // this.driverAge("驾驶机动车颜色统计","carColor",[20,54,50,43, 64,53],["红","黑","蓝","白","灰","银白"]);
    },
    // 违法机动车使用性质统计
    carUser(timer) {
      let allUseNature = this.$store.state.useNature;
          let req = TrafficOrderAnalysis.useNature.data;
          let driverLocation = [];
          let dataY = [];
          let dataS = [];
          if(req.length > 0) {
            req.map(val => {
              ...
              this.$showSimpleMessage("违法机动车使用性质统计暂无数据", "error");
          };            
          let start = 0;
          let length = dataS.length;
          let dataO,dataT;
          ...
          let use = setInterval(()=>{
              if(this.carNatureScroll) {
                  if(start+6 === length || start+6 > length) {
                    ...
                    dataO = this.$tools.deepCopy(dataS).splice(start,5);
                    dataT = this.$tools.deepCopy(dataY).splice(start,5);
                    this.driverAge("违法机动车使用性质统计（辆）","carNature",dataO,dataT,"辆");
              };
          },2000);
          this.use = use;   
    },
    stopCarNature() {
        // console.log("carNatureScroll")
    ...
    carProperties(timer) {
      let allVehicleUse = this.$store.state.vehicleUse;
          let req = TrafficOrderAnalysis.vehicleUse.data;
          let driverLocation = [];
          let dataY = [];
          let dataS = [];
          if(req.length > 0) {
              ...
              this.$showSimpleMessage("违法机动车用途性质统计暂无数据", "error");
          };         
          this.driverAge("违法机动车用途性质统计（辆）","carUse",dataS,dataY,"辆");
        //   this.barCanvas(
        //     "违法机动车用途性质统计",
        //     dataY,
        //     dataS,
    ...
    // 违法机动车违法次数统计
    illNum(timer) {
          let req = TrafficOrderAnalysis.carVioNum.data;
          let driverLocation = [];
          let dataY = [];
          let dataS = [];
          if(req.length > 0) {
              ...
              this.$showSimpleMessage("违法机动车违法次数统计暂无数据", "error");
          };          
        //   this.barCanvas(
        //     "违法机动车违法次数统计",
        //     dataY,
        ...
        //     "#eb6fe9"
        //   );
          this.driverTime("illNum", "违法机动车违法次数统计（辆）",dataS, dataY,["#206fc0", "#20c09c", "#247e44", "#d18523", "#6b20c0"]);
    },
    // 违法机动车事故次数统计
    illAccidentNum(timer) {
          let req = TrafficOrderAnalysis.carAcdNum.data;
          let driverLocation = [];
          let dataY = [];
          let dataS = [];
          if(req.length > 0) {
              ...
              this.$showSimpleMessage("违法机动车事故次数统计暂无数据", "error");
          };           
          this.driverTime("accidentNum", "违法机动车事故次数统计（辆）",dataS, dataY,["#206fc0", "#20c09c", "#247e44", "#d18523", "#6b20c0"]);
    },
    // 涉事机动车所有权统计结果
    carBlone(timer) {
      let ownershipList = this.$store.state.Ownership;
          let req = TrafficOrderAnalysis.ownership.data;
          let driverLocation = [];
          let dataY = [];
          let dataS = [];
          if(req.length > 0) {
    ...
    stopInter() {
        this.ifStopInter = true;
    },
    keepInter() {
        this.ifStopInter = false;
    }
  },
  beforeMount() {
        window.clearInterval(this.aa);
  ...
  async mounted() {
      if (this.$store.state.isExport) {
          ...
          await this.getTrafficDay(this.timer);
          ...
          await this.carBlone(this.timer);
          this.scrollTop(28);
          this.setIn = setInterval(()=>{
              window.clearInterval(this.ww);
              window.clearInterval(this.dd);
