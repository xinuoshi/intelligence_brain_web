<template>
  ...
  <div class="maincss">
    <div style="position:absolute;top:120px;bottom:0px;overflow: auto;padding: 0px 15px;margin-top: 10px">
      ...
      <span class="maincssall"
            style="background-color: #0D1E35">
        <el-carousel class="maintop"
                     :interval="5000"
                     indicator-position="none"
                     ref="zpEarchl">
          <el-carousel-item class="maintwo">
            <div style="width: 530px;height:100%;padding: 0 30px;"
                 class="zpEarchl">
              <div class="titileName">电子警察覆盖率</div>
              <div class="flexRow"
                   v-for="(item, index) in peiList1"
                   :key="index">
                <div v-for="(items, i) in item.value"
                     :key="i"
                     style="margin-left: 2vw">
                  <el-progress class="yhcolor"
                               max=100
                               type="circle"
                               :stroke-width=10
                               :width="wid"
                               :percentage="Number(items.pei)"
                               v-bind:class="{ colordi: Number(items.pei)>100 }"
            ...
            <div id="Columns2"
                 class="zpEarchl"
                 style="width: calc(100% - 500px);height:100%"></div>
          </el-carousel-item>
          <el-carousel-item class="mainthree">
            <div style="width: 530px;height:100%;padding: 0 30px;"
                 class="zpEarchl">
              <div class="titileName">人行灯覆盖率</div>
              <div class="flexRow"
                   v-for="(item, index) in peiList2"
                   :key="index">
                <div v-for="(items, i) in item.value"
                     :key="i"
                     style="margin-left: 2vw">
          ...
          <el-carousel-item class="mainfourl">
            <div style="width: 530px;height:100%;padding: 0 30px;"
                 class="zpEarchl">
              <div class="titileName">斑马线覆盖率</div>
              <div class="flexRow"
                   v-for="(item, index) in peiList3"
                   :key="index">
                <div v-for="(items, i) in item.value"
                     :key="i"
                     style="margin-left: 2vw">
        ...
        <div class="mainfive">
          <div id="lineColumns3"
               class="zpEarchl"
               style="width: 60%;height:100%"></div>
          <div id="lineColumns4"
               class="zpEarchl"
               style="width: 39.9%;height:100%">
            <div class="FacilityTable">
              ...
              <el-table :data="tableData"
                ...
                <el-table-column prop="rank"
                                 label="排行"> </el-table-column>
                <el-table-column prop="roadName"
                                 label="道路"> </el-table-column>
              </el-table>
            </div>
          </div>
        </div>
...
import facilityOperationAnalysis from '../../../JSON/facilityOperationAnalysis'
...
export default {
  ...
  data() {
    return {
      ...
      timedatall: this.$filter.formatDateTimeDay(new Date().getTime()),
      tableData: [
        {
          rank: '100',
          date: '1',
      ...
      /*// 搜索传参
                searchName: [
                    {searchPlaceholder: "查询区域", id: "", type: 1, url: "", Sendkey: "ares", data: []},
                    {searchPlaceholder: "查询日期", id: "", type: 3, url: "", Sendkey: "startTime", data: "", time: ""},
                    {searchPlaceholder: "止", id: "", type: 4, url: "", Sendkey: "endTime", data: "", time: ""}
                ],*/
      // 按钮传参
      UnitTopButton: [
        {
  ...
  methods: {
    ...
    timesevenl() {
      ...
      let hourseven =
        this.$filter.timesFormat(timeStamp) - 7 * 24 * 60 * 60 * 1000
      return hourseven
    },
    doHandleMonth(month) {
      var m = month
    ...
    searchEvent(payload) {
      ...
      let timer = {
        ...
        endTime:
          payload.endTime === undefined
            ? ''
            : this.$filter.timesFormat(new Date(payload.endTime)),
        startTimel:
        ...
        startTime:
          payload.startTime === undefined
            ? ''
            : this.$filter.timesFormat(new Date(payload.startTime)),
        area: payload.ares === undefined ? '' : payload.ares,
        equipmentType:
          payload.equipmentType === undefined ? '' : payload.equipmentType
      }
      this.time = timer
    ...
    fugailv_d() {
      let peiList = []
      var arr = this.getRandomNum(100, 8)
      console.log(arr)
      peiList.push(
        {
          value: [
            {
              ...
              pei: Number(this.tablevalue[0].electronicPoliceCR),
              color: 'green'
            },
            {
              name: this.tablevalue[1].areaName,
              pei: Number(this.tablevalue[1].electronicPoliceCR),
              color: '#8e71c7'
            },
            {
              name: this.tablevalue[2].areaName,
              pei: Number(this.tablevalue[2].electronicPoliceCR),
              color: '#C78C34'
            },
            {
              name: this.tablevalue[3].areaName,
              pei: Number(this.tablevalue[3].electronicPoliceCR),
              color: 'red'
            }
          ]
        },
          ...
          value: [
            {
              ...
              pei: Number(this.tablevalue[4].electronicPoliceCR),
              color: 'green'
            },
            {
              name: this.tablevalue[5].areaName,
              pei: Number(this.tablevalue[5].electronicPoliceCR),
              color: '#8e71c7'
            },
            {
              name: this.tablevalue[6].areaName,
              pei: Number(this.tablevalue[6].electronicPoliceCR),
              color: '#C78C34'
            },
            {
              name: this.tablevalue[7].areaName,
              pei: Number(this.tablevalue[7].electronicPoliceCR),
              color: 'red'
            }
          ]
        }
      ...
      peiList.push(
        {
          value: [
            {
              name: this.tablevalue[0].areaName,
              pei: Number(this.tablevalue[0].pedestrianLightCR),
              color: 'green'
            },
            {
              name: this.tablevalue[1].areaName,
              pei: Number(this.tablevalue[1].pedestrianLightCR),
              color: '#8e71c7'
            },
            {
              name: this.tablevalue[2].areaName,
              pei: Number(this.tablevalue[2].pedestrianLightCR),
              color: '#C78C34'
            },
            {
              name: this.tablevalue[3].areaName,
              pei: Number(this.tablevalue[3].pedestrianLightCR),
              color: 'red'
            }
          ]
        },
        {
          value: [
            {
              name: this.tablevalue[4].areaName,
              pei: Number(this.tablevalue[4].pedestrianLightCR),
              color: 'green'
            },
            {
              name: this.tablevalue[5].areaName,
              pei: Number(this.tablevalue[5].pedestrianLightCR),
              color: '#8e71c7'
            },
            {
              name: this.tablevalue[6].areaName,
              pei: Number(this.tablevalue[6].pedestrianLightCR),
              color: '#C78C34'
            },
            {
              name: this.tablevalue[7].areaName,
              pei: Number(this.tablevalue[7].pedestrianLightCR),
              color: 'red'
            }
          ]
        }
      ...
      peiList.push(
        {
          value: [
            {
              name: this.tablevalue[0].areaName,
              pei: Number(this.tablevalue[0].zebraCrossingCR),
              color: 'green'
            },
            {
              name: this.tablevalue[1].areaName,
              pei: Number(this.tablevalue[1].zebraCrossingCR),
              color: '#8e71c7'
            },
            {
              name: this.tablevalue[2].areaName,
              pei: Number(this.tablevalue[2].zebraCrossingCR),
              color: '#C78C34'
            },
            {
              name: this.tablevalue[3].areaName,
              pei: Number(this.tablevalue[3].zebraCrossingCR),
              color: 'red'
            }
          ]
        },
        {
          value: [
            {
              name: this.tablevalue[4].areaName,
              pei: Number(this.tablevalue[4].zebraCrossingCR),
              color: 'green'
            },
            {
              name: this.tablevalue[5].areaName,
              pei: Number(this.tablevalue[5].zebraCrossingCR),
              color: '#8e71c7'
            },
            {
              name: this.tablevalue[6].areaName,
              pei: Number(this.tablevalue[6].zebraCrossingCR),
              color: '#C78C34'
            },
            {
              name: this.tablevalue[7].areaName,
              pei: Number(this.tablevalue[7].zebraCrossingCR),
              color: 'red'
            }
          ]
        }
    ...
    getParams(timer) {
      ...
      if (!timer) {
        ...
        ;(area = undefined), (equipmentType = undefined)
      } else {
    ...
    getTrafficNumber(timer) {
      ...
      let tablevalue = facilityOperationAnalysis.reportForm.data
      let areaName = []
      ...
      tablevalue.map(val => {
        ...
        totalOutfieldEquipmentRingRatio.push(
          val.totalOutfieldEquipmentRingRatio
        )
        totalOutfieldEquipmentSum.push(val.totalOutfieldEquipmentSum)
        totalOutfieldEquipmentYearOnYear.push(
          val.totalOutfieldEquipmentYearOnYear
        )
    ...
    duration(timer) {
      ...
      let tablevalue = facilityOperationAnalysis.acdRoad.data
  ...
  mounted() {
    ...
    let timer = {
      endTime: this.timedata,
      startTime: this.timesevenl(),
      endTimel: this.thistime,
      startTimel: this.timeseven(-7),
      area: '',
      equipmentType: ''
    }
    this.time = timer
    if (this.$store.state.isExport) {
      ...
      let timerr = {
        endTimel:
          aa.endTime === undefined
            ? ''
            : this.$tools.getDays(new Date(aa.endTime)),
        startTimel:
          aa.startTime === undefined
            ? ''
            : this.$tools.getDays(new Date(aa.startTime)),
        area: aa.ares === undefined ? '' : aa.ares,
        equipmentType: aa.equipmentType === undefined ? '' : aa.equipmentType
      }
      this.mapFullscreenLoding()
      // 自制等待圈
      this.searchName[1].time = this.$filter.formatDateTimeDay(
        this.$store.state.isExportParams.startTime
      )
      this.searchName[2].time = this.$filter.formatDateTimeDay(
        this.$store.state.isExportParams.endTime
      )
      this.t = setInterval(() => {
        this.getTrafficNumber(this.$store.state.isExportParams)
        this.duration(timerr)
      }, 180000)
  ...
  created() {
    ...
    this.searchName[1].time = this.timesevenl(-7)
    this.searchName[2].time = this.timedatall
  },
  watch: {
    changeDivW: {
