...
<script>
    export default {
        ...
        watch: {
            "$route"(to, from) {
                if (to.path === "/InternalManagementAnalysis" || from.path === "/InternalManagementAnalysis") {
                    this.upIsIndex = 0;
                    setTimeout(() => {
        ...
        data() {
            return {
                ...
                lowerMenuBarListop: [
                    { name: "设施运行分析" ,
                        equipmentTypeList: [
                            {name:"设施运行分析", path: "/FacilityOperationAnalysis"}
                        ]
                    }
                ],
                lowerMenuBarList: [],
        ...
        created() {
            ...
            if (item) {
                let otem = item.nextlevel.find(e => e.name === "内部管理分析");
                if (otem) {
