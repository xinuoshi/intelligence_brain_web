 ...
 * @Last Modified time: 2019-01-04 17:19:01
 * shullet
 */
        ...
        <el-dialog :title="WireList.title" v-dialogDrag :visible.sync="WireList.ifshow" class="tems_dialog WireFrame" width="1440px" :before-close="closeDialog" :close-on-press-escape="false" :close-on-click-modal="false" >
            <el-form :model="formValidate" :rules="ruleValidate" ref="formValidate" class="demo-ruleForm">
                ...
                <div class="canvasBox" >
                    ...
                    <!-- <video autoplay = "autoplay" loop="loop" :src="this.$store.state.videoUrl" style="position: absolute;object-fit: fill;" width="1000px" height = "500px">
                    </video> -->
                    <video id="AIvideo" class="video-js"></video>
...
export default {
    name: "Wire",
    props: ["WireList", "incidentNum","rtspAddr"],
    model: {
        click: "WireButton"
    },
    data() {
    ...
    watch: {
        "WireList.id": {
            handler: function(val, oldVal) {
                ...
                if (oldVal && val !== "") {
                    this.video.src({ src: `http://${this.GLOBAL.serviceIp}/wit_harns/mp4/${this.rtspAddr}`, type: "video/mp4",});
                    this.video.load();
                    this.video.play();
                ...
                if (val === true) {
                    ...
                    setTimeout(() => {
                        video = this.$videojs(`#AIvideo`, {
                            ...
                            sources: [
                                {
                                src: `http://${this.GLOBAL.serviceIp}/wit_harns/mp4/${this.rtspAddr}`,
                                type: "video/mp4",
                                }
                            ],
                        });
