 ...
 * @Last Modified time: 2019-01-23 14:34:07
 */
<template>
    <div :class="tdClass">
                ...
                <td :colspan="colSpan">
                    <table>
                        <tr class="leve" :class="levelClass">
                            ...
                            <td class="td3">
                                <span :title="model.t.contactPhone" class="ellipsis">{{model.t.contactPhone}}</span>
                            </td>
                            <td class="td4" style="text-align: center;">
                                <span :title="model.t.contactEmail" class="ellipsis">{{model.t.contactEmail || "无"}}</span>
                            </td>
                            <td class="td5">
                            ...
                            <td class="td6">
                                ...
                                <i class="line" v-if="viewUnitFuncButton && (actionFuncButton || addingFuncButton || deleteFuncButton)"></i>
                                <a class="reset" href="javascript:;" v-if="actionFuncButton" @click="actionFunc(model)">编辑</a>
                                <i class="line" v-if="actionFuncButton"></i>
                                <a class="reset" href="javascript:;" v-if="addingFuncButton" @click="addingFunc(model)">添加</a>
                                <i class="line" v-if="addingFuncButton"></i>
...
export default {
    ...
    computed: {
        ...
        // 查看按钮
        viewUnitFuncButton() {
            let item = this.UnitTabularButton.find(t => t.name === "查看");
            if (item) {
                return true;
            }
            return false;
        ...
        disableOperationFuncButton() {
            let item = this.UnitTabularButton.find(t => t.name === "启用" || t.name === "停用");
            if (item) {
                return true;
            }
            return false;
        ...
        // 编辑按钮
        actionFuncButton() {
            let item = this.UnitTabularButton.find(t => t.name === "编辑");
            if (item) {
                return true;
            }
            return false;
        ...
        // 添加按钮
        addingFuncButton() {
            let item = this.UnitTabularButton.find(t => t.name === "添加");
            if (item) {
                return true;
            }
            return false;
        ...
        // 删除按钮
        deleteFuncButton() {
            let item = this.UnitTabularButton.find(t => t.name === "删除");
            if (item) {
                return true;
            }
            return false;
